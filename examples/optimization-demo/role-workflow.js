...
function bytes(v){return v instanceof Uint8Array?v:new Uint8Array(v)}
...
// ---- 大对象存储:sessionStorage 只放小 metadata,.enc 包存 IndexedDB ----
const idb={db:null,
  open(){return new Promise((res,rej)=>{if(idb.db)return res(idb.db);const r=indexedDB.open('soci-demo-pkgs',1);r.onupgradeneeded=()=>r.result.createObjectStore('pkgs');r.onsuccess=()=>{idb.db=r.result;res(idb.db)};r.onerror=()=>rej(r.error)})},
  async put(k,v){const db=await idb.open();return new Promise((res,rej)=>{const t=db.transaction('pkgs','readwrite');t.objectStore('pkgs').put(v,k);t.oncomplete=res;t.onerror=()=>rej(t.error)})},
  async get(k){const db=await idb.open();return new Promise((res,rej)=>{const r=db.transaction('pkgs').objectStore('pkgs').get(k);r.onsuccess=()=>res(r.result||null);r.onerror=()=>rej(r.error)})},
  async clear(){const db=await idb.open();return new Promise((res,rej)=>{const t=db.transaction('pkgs','readwrite');t.objectStore('pkgs').clear();t.oncomplete=res;t.onerror=()=>rej(t.error)})}};

// ---- 业务流程状态机:A 发起 → C 加密 → A 委托 → B 计算 → B 返回 → A 提交 → C 授权 → A 接收 ----
const STORE='soci-role-flow-v2';
...
let flow=freshFlow(),plainRows=null,plainName='',computing=false,runtimeMode='';
const STEPS=[
...
const pkgKey=stage=>`${flow.task?flow.task.task_id:'-'}:${stage}`;
function liteManifest(m){if(!m)return null;const{encrypted_result,...rest}=m;return rest}
function persist(){try{sessionStorage.setItem(STORE,JSON.stringify({task:flow.task,cipher:liteManifest(flow.cipher&&flow.cipher.manifest),delegation:flow.delegation?flow.delegation.manifest:null,computed:liteManifest(flow.computed&&flow.computed.manifest),returned:flow.returned,submitted:!!flow.submitted,authorized:flow.authorized,final:flow.final}))}catch(e){console.warn('流程状态持久化失败',e)}}
async function restore(){try{const s=sessionStorage.getItem(STORE);if(!s)return;const f=JSON.parse(s);if(!f||typeof f!=='object')return;
  flow=freshFlow();flow.task=f.task||null;flow.returned=!!f.returned;flow.authorized=f.authorized||null;flow.final=!!flow.authorized&&!!f.final;
  const tid=flow.task?flow.task.task_id:'-';
  if(f.cipher)flow.cipher={manifest:f.cipher,pkg:await idb.get(`${tid}:cipher`)};
  if(f.delegation)flow.delegation={manifest:f.delegation,pkg:await idb.get(`${tid}:delegation`)};
  if(f.computed)flow.computed={manifest:f.computed,pkg:await idb.get(`${tid}:result`)};
  if(f.submitted&&flow.computed)flow.submitted=flow.computed;
}catch(e){console.warn('流程状态恢复失败',e)}}
...
// fail-closed:stage/format/result_id/task_id 任一不匹配即拒绝
function expectCurrentTask(taskId,what){if(!flow.task)throw Error('请先由 A 发起当前任务');if(!taskId||taskId!==flow.task.task_id)throw Error(`文件不属于当前任务:${what} task_id 不匹配`)}
function render(){
  ...
  setCard('#cDecryptCard',!!flow.submitted,!!flow.authorized,!flow.submitted?'等待 A 提交':flow.authorized?'已授权，待 A 接收':'可授权');
  setCard('#aFinalCard',!!flow.authorized,!!flow.final,!flow.authorized?'等待 C 授权':flow.final?'已完成':'待 A 确认接收');
  ...
  check('#aFinalNotice',flow.authorized&&!flow.final&&`✓ C 已签发授权结果 · 任务 ${flow.authorized.task_id||'-'} · 请确认接收`);
  $('#bParams').innerHTML=flow.delegation?`<span>阈值 ≥ <b>${esc(flow.delegation.manifest.threshold)}</b></span><span>策略 <b>${esc(flow.delegation.manifest.strategy)}</b></span><span><b>K = ${esc(flow.delegation.manifest.grid||3)}</b></span><span><b>${esc(flow.delegation.manifest.row_count)}</b> 行物料</span>`:'';
...
// ---- B 方长任务 ETA:只用公开特征(rows/有效成本数/strategy/K/mode/历史实测) ----
const HIST='soci-demo-compute-hist-v1';
function loadHist(){try{return JSON.parse(localStorage.getItem(HIST)||'[]').filter(h=>h&&h.ms>0)}catch(e){return[]}}
function recordHist(h){try{const a=loadHist();a.push(h);localStorage.setItem(HIST,JSON.stringify(a.slice(-40)))}catch(e){}}
function predictMs(cells,strategy,grid){
  const h=loadHist();
  const same=h.filter(x=>x.mode===runtimeMode&&x.strategy===strategy&&x.grid===grid);
  const pool=same.length?same:h.filter(x=>x.mode===runtimeMode);
  if(!pool.length)return null;   // 无历史:不编造精确 ETA
  const per=pool.map(x=>x.ms/Math.max(1,x.cells)).sort((a,b)=>a-b);
  return per[Math.floor(per.length/2)]*Math.max(1,cells);
}
function fmtDur(s){s=Math.max(0,Math.round(s));return s<60?`${s}s`:`${Math.floor(s/60)}m ${String(s%60).padStart(2,'0')}s`}
let ticker=null;
function startProgress(estMs){stopProgress();const t0=performance.now();let shown=0;
  const bar=$('#bProgress');bar.classList.remove('hidden','done');bar.classList.toggle('estimating',estMs==null);$('#cpFill').style.width='0%';
  ticker=setInterval(()=>{const t=(performance.now()-t0)/1000;
    $('#cpElapsed').textContent=`已用 ${fmtDur(t)}`;
    if(estMs==null){$('#cpPct').textContent='—';$('#cpEta').textContent='正在估算耗时';$('#cpPhase').textContent='正在进行密态最优化计算';return}
    let est=estMs/1000;if(t>est)est=est+(t-est)*1.5;   // 超出预测:动态扩大总耗时与剩余时间
    shown=Math.max(shown,Math.min(95,92*t/est));        // 单调不倒退,完成前封顶 95%
    $('#cpFill').style.width=shown.toFixed(1)+'%';$('#cpPct').textContent=Math.floor(shown)+'%';
    $('#cpEta').textContent=`预计剩余 ${fmtDur(est-t)}`;
...
function finishProgress(t0){stopProgress();const t=(performance.now()-t0)/1000;const bar=$('#bProgress');bar.classList.remove('estimating');bar.classList.add('done');$('#cpFill').style.width='100%';$('#cpPct').textContent='100%';$('#cpElapsed').textContent=`已用 ${fmtDur(t)}`;$('#cpEta').textContent='计算完成';$('#cpPhase').textContent='密态计算完成'}
...
$('#aStart').onclick=()=>{const t=Number($('#aThreshold').value);if(!(t>=0&&t<1))return showError(Error('阈值必须满足 0 ≤ T < 1'));
  ...
  idb.clear().catch(()=>{});persist();render();goto('owner')};
...
$('#cEncrypt').onclick=async()=>{const btn=$('#cEncrypt');if(!flow.task||!plainRows)return;btn.disabled=true;btn.firstChild.textContent='正在加密… ';
  try{const d=await api('/api/workflow/encrypt',wire(plainRows,flow.task));
    const cells=plainRows.reduce((n,x)=>n+x.costs.filter(v=>v!=null).length,0);
    const manifest={format:'SOCI-ROLE-ENC',version:1,stage:'encrypted-input',job_id:d.jobId,row_count:plainRows.length,cell_count:cells,algorithm:'Paillier-3072',task:flow.task,created_by:'role-C',created_at:new Date().toISOString()};
    ...
    const pkg=makeEnc(manifest,rows);flow.cipher={manifest,pkg};
    await idb.put(pkgKey('cipher'),pkg).catch(()=>{});
...
wireDrop($('#aCipherDrop'),$('#aCipherFile'),async file=>{const{manifest,bytes:raw}=await readPkg(file);if(manifest.stage!=='encrypted-input')throw Error('文件不属于当前任务:需要 C 生成的加密成本文件');expectCurrentTask(manifest.task&&manifest.task.task_id,'加密文件');flow.cipher={manifest,pkg:raw};await idb.put(pkgKey('cipher'),raw).catch(()=>{});persist();render()});
$('#aDelegate').onclick=async()=>{try{if(!flow.task||!flow.cipher)return;if(!flow.cipher.pkg)throw Error('密文包数据不可用，请重新导入 C 的加密文件');const c=flow.cipher.manifest;
    const manifest={format:'SOCI-ROLE-ENC',version:1,stage:'delegated-task',job_id:c.job_id,row_count:c.row_count,cell_count:c.cell_count,algorithm:c.algorithm,task_id:flow.task.task_id,threshold:flow.task.threshold,strategy:flow.task.strategy,grid:flow.task.grid,delegated_by:'role-A',delegated_at:new Date().toISOString()};
    const pkg=await repack(flow.cipher.pkg,manifest);flow.delegation={manifest,pkg};
    await idb.put(pkgKey('delegation'),pkg).catch(()=>{});
    download(pkg,`soci-A-delegate-${flow.task.task_id}.enc`);persist();render();goto('compute');
  }catch(e){showError(e)}};
wireDrop($('#bTaskDrop'),$('#bTaskFile'),async file=>{const{manifest,bytes:raw}=await readPkg(file);if(manifest.stage!=='delegated-task')throw Error('文件不属于当前任务:需要 A 生成的委托文件');expectCurrentTask(manifest.task_id,'委托文件');flow.delegation={manifest,pkg:raw};await idb.put(pkgKey('delegation'),raw).catch(()=>{});persist();render()});
async function runCompute(){const btn=$('#bCompute');if(!flow.delegation||computing)return;const m=flow.delegation.manifest;
  ...
  const cells=m.cell_count||m.row_count*3;
  const t0=startProgress(predictMs(cells,m.strategy,m.grid||3));
  try{const d=await api('/api/workflow/compute',`${m.job_id}\t${m.threshold}\t${m.strategy}\t${m.grid||3}`);
    const ms=performance.now()-t0;
    recordHist({rows:m.row_count,cells,strategy:m.strategy,grid:m.grid||3,mode:d.mode,ms:Math.round(ms)});
    finishProgress(t0);
    const manifest={format:'SOCI-ROLE-ENC',version:1,stage:'computed-result',job_id:m.job_id,result_id:d.resultId,task_id:m.task_id,threshold:m.threshold,strategy:m.strategy,grid:m.grid||3,mode:d.mode,stats:d.stats,first_cipher:d.ciphertexts[0],encrypted_result:d.ciphertexts,computed_by:'role-B',computed_at:new Date().toISOString()};
    const pkg=makeEnc(manifest,[['RESULT_COLUMN'],...d.ciphertexts.map(x=>[x])]);
    flow.computed={manifest,pkg};
    await idb.put(pkgKey('result'),pkg).catch(()=>{});
...
$('#bExport').onclick=()=>{if(!flow.computed)return;if(!flow.computed.pkg)return showError(Error('结果包数据不可用，请重新导入 B 的结果文件'));download(flow.computed.pkg,`soci-B-result-${flow.computed.manifest.task_id||'out'}.enc`);flow.returned=true;persist();render();goto('user')};
wireDrop($('#aResultDrop'),$('#aResultFile'),async file=>{const{manifest,bytes:raw}=await readPkg(file);if(manifest.stage!=='computed-result'||!manifest.result_id)throw Error('文件不属于当前任务:需要 B 返回的计算结果文件');expectCurrentTask(manifest.task_id,'结果文件');flow.computed={manifest,pkg:raw};flow.returned=true;await idb.put(pkgKey('result'),raw).catch(()=>{});persist();render()});
$('#aSubmit').onclick=()=>{if(!flow.computed||!flow.returned)return;flow.submitted=flow.computed;persist();render();goto('owner')};
wireDrop($('#cResultDrop'),$('#cResultFile'),async file=>{const{manifest,bytes:raw}=await readPkg(file);if(manifest.stage!=='computed-result'||!manifest.result_id)throw Error('文件不属于当前任务:需要 A 提交的计算结果文件');expectCurrentTask(manifest.task_id,'结果文件');flow.computed=flow.computed||{manifest,pkg:raw};flow.returned=true;flow.submitted={manifest,pkg:raw};await idb.put(pkgKey('result'),raw).catch(()=>{});persist();render()});
...
$('#aConfirm').onclick=()=>{if(!flow.authorized)return;flow.final=true;persist();render()};
wireDrop($('#aFinalDrop'),$('#aFinalFile'),async file=>{const d=JSON.parse(td.decode(new Uint8Array(await file.arrayBuffer())));if(d.format!=='SOCI-AUTHORIZED-RESULT'||!Array.isArray(d.solution))throw Error('文件不属于当前任务:需要 C 签发的授权结果文件');expectCurrentTask(d.task_id,'授权结果');flow.authorized=d;flow.final=true;persist();render()});
...
fetch('/api/health').then(r=>r.json()).then(x=>{runtimeMode=x.mode;$$('[data-runtime-mode]').forEach(e=>e.textContent=x.mode)}).catch(()=>{});
restore().then(render);
