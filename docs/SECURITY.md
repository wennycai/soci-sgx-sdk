...
scalar multiply, and full decryption. The reference SMUL assumes semi-honest
parties. SSBA reveals its decrypted input to CSP; SCMP consequently reveals a
comparison difference, and SDIV repeatedly reveals subtraction predicates.
They lack replay protection and a formal malicious-security proof. All
`secure_*` entry points therefore return `SOCI_INVALID_STATE` by default and
are labelled experimental/reference-only.
